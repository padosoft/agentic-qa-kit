...
export class InMemoryGiftCardLedger implements GiftCardLedger {
  ...
  async credit(
    ...
    const prior = this.entries.get(operationId);
...
export class PostgresGiftCardLedger implements GiftCardLedger {
  ...
  async credit(
    ...
    return this.sql.begin(async (tx) => {
      ...
      const prior = await findOperation(query, operationId);
      if (prior) return sameEntry(prior, entry) ? 'duplicate' : 'conflict';
...
async function findOperation(
  ...
  const rows = (await query(
    'SELECT tenant, gift_card_id, currency, amount_minor::text AS amount_minor, kind FROM aqa_commerce_gift_card_entries WHERE operation_id = $1',
    [operationId],
  )) as Array<{
    tenant: string;
    gift_card_id: string;
