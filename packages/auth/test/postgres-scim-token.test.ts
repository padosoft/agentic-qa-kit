...
describe('PostgresScimTokenStore', () => {
  it('persists hash-only SCIM tokens across store instances', async () => {
...
describe('PostgresSamlReplayGuard', () => {
  it('claims an assertion once across store instances', async () => {
    const dsn = process.env.AQA_TEST_POSTGRES_DSN;
    if (!dsn) {
      console.warn('SKIP: AQA_TEST_POSTGRES_DSN is required for the live PostgreSQL contract');
      return;
...
describe('PostgresScimRateLimiter', () => {
  it('shares an atomic tenant window across limiter instances', async () => {
    const dsn = process.env.AQA_TEST_POSTGRES_DSN;
    if (!dsn) {
      console.warn('SKIP: AQA_TEST_POSTGRES_DSN is required for the live PostgreSQL contract');
      return;
...
describe('PostgresMfaCredentialStore', () => {
  it('persists tenant-scoped protected MFA metadata across store instances', async () => {
    const dsn = process.env.AQA_TEST_POSTGRES_DSN;
    if (!dsn) {
      console.warn('SKIP: AQA_TEST_POSTGRES_DSN is required for the live PostgreSQL contract');
      return;
