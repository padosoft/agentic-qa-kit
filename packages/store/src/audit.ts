...
export class InvalidFindingVerificationError extends Error {
  readonly code = 'INVALID_FINDING_VERIFICATION';

  constructor(message: string) {
    super(message);
    this.name = 'InvalidFindingVerificationError';
  }
}
...
export function findingStatusAudit(
  ...
  const event = {
    ...
    kind: 'finding_status_changed' as const,
...
export function findingVerificationAudit(
  finding: Finding.Finding,
  actor: string,
  ...
  const event = {
    ...
    ts: new Date().toISOString(),
    run_id: finding.run_id,
    kind: 'finding_verification_recorded' as const,
    actor: { type: 'system' as const, id: actor },
    finding_id: finding.id,
    payload: {
      action: 'finding_verification_recorded',
      from,
      to,
