...
| S-03 | Pack-author spoofing (malicious pack pretending to be `@aqa/...`) | Critical | Enterprise import boundaries enable `scanPack(..., { requireSignature: true })`; trusted Ed25519/Sigstore verification remains configurable through the operator trust root. Local authoring can explicitly retain unsigned/dev mode. | **Mitigated at the configured import boundary; trust-root operations and signed release artifacts remain deployment evidence.** |
