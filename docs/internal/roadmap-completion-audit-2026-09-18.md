...
| v2.2 Stateful agentic QA | `partial` | Capability preflight, bounded state graph validation, temporal/stateful journey primitives, multi-actor context, evidence boundaries and bounded OTLP trace federation exist. Runtime journey compilation/execution, mutation-to-regression automation and production-grade trace/provenance execution are not yet complete. |
