...
| v0.6 Methodology rigor | `partial` | STRIDE/FMEA/OWASP, risk coverage, oracle safety/calibration, bounded mutation evidence and explainable same-risk semantic clustering exist. Protected mutation execution at scale, threshold calibration governance and independent methodology validation remain open. |
...
| v2.2 Stateful agentic QA | `partial` | Capability preflight, bounded state graph validation, temporal/stateful journey primitives, multi-actor context and evidence boundaries exist. Runtime journey compilation/execution, mutation-to-regression automation and production-grade trace federation are not yet complete. |
