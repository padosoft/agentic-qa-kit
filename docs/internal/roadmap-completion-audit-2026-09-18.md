...
| v0.6 Methodology rigor | `partial` | STRIDE/FMEA/OWASP, risk coverage, oracle safety/calibration, bounded mutation evidence, mutation-to-regression coverage, explainable same-risk semantic clustering and reviewed-pair threshold calibration exist. Protected mutation execution at scale, holdout/independent methodology validation and external governance remain open. |
...
| v2.2 Stateful agentic QA | `partial` | Capability preflight, bounded state graph validation, actor-bound runtime journey compilation/execution, CLI/worker lifecycle integration, temporal observers, safe journey-to-trace correlation, evidence boundaries and bounded OTLP trace federation exist. Mutation-to-regression automation and production-grade trace/provenance execution are not yet complete. |
