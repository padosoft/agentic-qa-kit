...
bytes. Subscriber failures are isolated. Callers must persist authoritative
state before publishing and reconcile from the store after reconnects.

## Consequences

- Replicas receive low-latency fan-out without introducing Redis/Kafka.
- Delivery is explicitly non-durable and at-most-once; it is unsuitable for
  job execution or audit storage.
- The same consumer contract can later sit behind Redis Streams/Kafka without
  changing API handlers.
- Production operators must provide PostgreSQL connectivity and reconnect
  reconciliation at the consumer boundary.
